...
║   /dcaon     — enable DCA (buy more on dips)                 ║
║   /dcaoff    — disable DCA                                   ║
║   /dcastatus — show current DCA state                        ║
...
# When enabled, the bot buys more BTC every time price drops by
# DCA_DROP_PCT below your last buy price, up to DCA_MAX_ORDERS times.
# Example: buy at $80k, price drops 3% to $77.6k → bot buys again
...
# DCA state — tracked per coin
# dca_orders[coin] = list of dicts with entry price and amount of each DCA buy
dca_orders       = {coin: [] for coin in COINS}
...
def tg_send(message):
    ...
    payload = {"chat_id": TELEGRAM_CHAT, "text": clean}
    try:
        if USE_REQUESTS:
...
def handle_command(text):
    ...
    """
    global is_paused, TAKE_PROFIT_PCT, STOP_LOSS_PCT, FNG_MIN_TO_BUY, DCA_ENABLED
    ...
    if cmd[0] == "/help":
        tg_send(
            ...
            "/setsl X — set stop loss % (e.g. /setsl 2)\n"
            "/setfng X — set min Fear & Greed (e.g. /setfng 45)\n"
            "/dcaon — enable DCA buying on dips\n"
            "/dcaoff — disable DCA\n"
            "/dcastatus — show DCA state and orders\n"
            "/stats — full session statistics\n"
            "/help — show this list"
        )
    ...
    # ── /dcaon ────────────────────────────────────────────────────
        ...
        tg_send(
            f"DCA enabled\n"
            f"Drop trigger: {DCA_DROP_PCT*100:.1f}%\n"
            f"Buy size: ${DCA_ORDER_SIZE*100:.0f} per dip\n"
            f"Max buys: {DCA_MAX_ORDERS}"
        )
        log("DCA enabled via Telegram.")

    # ── /dcaoff ───────────────────────────────────────────────────
    elif cmd[0] == "/dcaoff":
        DCA_ENABLED = False
        tg_send("DCA disabled. Bot will not buy on dips.")
        log("DCA disabled via Telegram.")

    # ── /dcastatus ────────────────────────────────────────────────
    elif cmd[0] == "/dcastatus":
        lines = [
            f"DCA Status: {'ON' if DCA_ENABLED else 'OFF'}",
            f"Drop trigger: {DCA_DROP_PCT*100:.1f}%",
            f"Buy size per dip: ${DCA_ORDER_SIZE*100:.0f}",
            f"Max buys per position: {DCA_MAX_ORDERS}",
            "",
        ]
        for coin in COINS:
            orders = dca_orders.get(coin, [])
            if orders:
                lines.append(f"{coin.split('/')[0]}: {len(orders)} DCA buys")
                for i, o in enumerate(orders):
                    lines.append(f"  Buy {i+1}: ${o['price']:,.2f}")
            else:
                lines.append(f"{coin.split('/')[0]}: no DCA buys yet")
        tg_send("\n".join(lines))
...
def connect():
    ...
    log("Kraken exchange initialised.")
...
def sell(exchange, coin, price, reason):
    ...
    positions[coin]  = None
    dca_orders[coin] = []  # Reset DCA orders on position close
...
def check_dca(exchange, coin, current_price):
    """
    Checks if a DCA buy should be placed.
    Only runs when DCA is enabled and we already have an open position.

    Logic:
    - Find the lowest price we last bought at (initial or last DCA)
    - If current price has dropped DCA_DROP_PCT below that price → buy more
    - Max DCA_MAX_ORDERS additional buys per position
    - Each buy uses DCA_ORDER_SIZE * starting balance ($10 on a $100 account)
    """
    global dca_orders

    if not DCA_ENABLED:
        return
    if positions[coin] is None:
        return
    if len(dca_orders[coin]) >= DCA_MAX_ORDERS:
        return

    # Reference price is the last buy price (initial entry or last DCA)
    if dca_orders[coin]:
        last_buy_price = dca_orders[coin][-1]["price"]
    else:
        last_buy_price = positions[coin]["entry"]
    ...
    # DCA buy triggered
    dca_num   = len(dca_orders[coin]) + 1
    spend_usd = round(100.0 * DCA_ORDER_SIZE, 2)  # Fixed $10 per DCA buy
    amount    = round(spend_usd / current_price, 6)

    log(f"DCA buy #{dca_num} triggered — price dropped "
        f"{drop*100:.2f}% from ${last_buy_price:,.4f}", coin)

    if SANDBOX:
        log(f"[SANDBOX] DCA BUY {amount} @ ${current_price:,.4f} "
            f"(${spend_usd:.2f})", coin)
    else:
        try:
            exchange.create_market_buy_order(coin, amount)
            log(f"DCA BUY {amount} @ ~${current_price:,.4f}", coin)
        except Exception as e:
            log(f"DCA buy failed: {e}", coin)
            return

    dca_orders[coin].append({
        "price":  current_price,
        "amount": amount,
        "usd":    spend_usd,
        "time":   datetime.now(),
    ...
    # Recalculate average entry across initial + all DCA buys
    all_prices  = [positions[coin]["entry"]] + [o["price"] for o in dca_orders[coin]]
    all_amounts = [positions[coin]["amount"]] + [o["amount"] for o in dca_orders[coin]]
    avg_entry   = sum(p * a for p, a in zip(all_prices, all_amounts)) / sum(all_amounts)

    tg_send(
        f"DCA Buy #{dca_num} — {coin.split('/')[0]}\n"
        f"Price:   ${current_price:,.4f}\n"
        f"Spent:   ${spend_usd:.2f}\n"
        f"Avg entry now: ${avg_entry:,.4f}\n"
        f"DCA buys: {len(dca_orders[coin])}/{DCA_MAX_ORDERS}\n"
        f"Mode: {'SANDBOX' if SANDBOX else 'LIVE'}"
    )
...
def process_coin(exchange, claude_client, coin, fng_value, fng_label):
    ...
    if should_exit:
        ...
        dca_orders[coin] = []  # Reset DCA orders when position closes
        trade_counts[coin] += 1
        return

    # DCA check — runs if we have an open position
    if positions[coin] is not None:
        check_dca(exchange, coin, price)
...
def run():
    ...
    # Tiny web server so Railway's health check passes
    def health_server():
